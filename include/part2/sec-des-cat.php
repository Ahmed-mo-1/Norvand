...
add_shortcode( 'afterca', 'bbloomer_display_wp_editor_content', 10 );
 
function bbloomer_display_wp_editor_content() {
   if ( is_product_taxonomy() ) {
      $term = get_queried_object();
      if ( $term && ! empty( get_woocommerce_term_meta( $term->term_id, 'seconddesc', true ) ) ) {
         echo '<div style="padding : 16px ; width: 90% !important; text-align : justify !important"><p style="" class="term-description">' . wc_format_content( htmlspecialchars_decode( get_woocommerce_term_meta( $term->term_id, 'seconddesc', true ) ) ) . '</p></div><br>';
      }
   }
}
?>