...
add_action( 'product_tag_add_form_fields', 'tag_add', 10, 2 );
...
add_action( 'product_tag_edit_form_fields', 'tag_edit', 10, 2 );
 
function tag_edit( $term ) {
...
add_action( 'edit_term', 'tag_save', 10, 3 );
add_action( 'created_term', 'tag_save', 10, 3 );
 
function tag_save( $term_id, $tt_id = '', $taxonomy = '' ) {
...
add_shortcode( 'pro_tags', 'tagcontent');
...
if ($posttags) {
    foreach($posttags as $product_tag) {
        ...
        echo wc_format_content( htmlspecialchars_decode( get_woocommerce_term_meta( $product_tag->term_id, 'seconddesc', true ) ) );
    }
}
      $term = get_queried_object();
      if ( $term && ! empty( get_woocommerce_term_meta( $term->term_id, 'seconddesc', true ) ) ) {
         echo '<p class="term-description">' . wc_format_content( htmlspecialchars_decode( get_woocommerce_term_meta( $term->term_id, 'seconddesc', true ) ) ) . '</p><br>';
      }
